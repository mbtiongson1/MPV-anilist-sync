...
- The default port is **8082** (or 8083 if 8082 is occupied).

## Browser Subagent Task

Navigate to `http://localhost:8082` (or the active port) and perform the following checks:
...
```markdown
Navigate to http://localhost:8082 and verify the "MPV AniList Tracker" header exists. Confirm the presence of the now-playing banner (idle or active) and the sidebar navigation. Report any missing key UI elements.
```
