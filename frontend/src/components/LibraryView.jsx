...
export function LibraryView() {
    ...
    // Filter library items by search using useMemo to prune the tree once per search/data change
    const filtered = useMemo(() => {
        if (!search) return data;
        const q = search.toLowerCase();

        // Recursive function to deeply prune the tree and return a new pruned structure
        const pruneTree = (items) => {
            ...
            return items.map(item => {
                ...
                if (item.children) {
                    newChildren = pruneTree(item.children);
                }

                // Keep item if its name matches OR if any of its children match
                if (nameMatch || (newChildren && newChildren.length > 0)) {
    ...
    const renderNode = (node, depth = 0) => {
        ...
        // Expand if it's root, or if user expanded, or if searching found matching children
        const expanded = depth === 0 || expandedDirs.has(node.path) || (search && node.children?.length > 0);
