...
export function NowPlaying({ onOpenDetails }) {
    ...
    // Optimization (Bolt): Wrap season options sorting in useMemo to prevent O(N log N) execution on every component render
    // Expected Impact: Reduces main thread blocking during frequent NowPlaying status updates (every 2s).
    // Note: Depends on data.season_options directly to avoid recreating empty array reference and causing redundant re-renders
    const seasonOptions = useMemo(() => {
        const rawOptions = Array.isArray(data.season_options) ? data.season_options : [];
        return [...rawOptions].sort((a, b) => (b.seasonYear || 0) - (a.seasonYear || 0));
    }, [data.season_options]);
