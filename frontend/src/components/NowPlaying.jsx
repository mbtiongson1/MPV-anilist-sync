...
import { latestStatus, userSettings, animeList, showToast, activeSearchTerm, recordApiRequest, setActiveTab, torrentCache, animeListMap } from '../store';
...
export function NowPlaying({ onOpenDetails }) {
    ...
    const handleSync = async () => {
        if (watched < 1) return;
        const anime = animeListMap.value.get(selectedMediaId);
        if (anime) {
