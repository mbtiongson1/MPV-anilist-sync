...
## 2026-07-09 - Fail-Open Path Validation in Config API
**Vulnerability:** The `/api/settings` endpoint failed to validate directory assignments (`default_download_dir` and `base_anime_folder`). An attacker could supply absolute paths (like `/etc/` or `C:\`) via the API, effectively bypassing all path traversal and `is_safe_path` protections which rely on these configuration values to define the "safe sandbox." The initial fix introduced a second vulnerability by wrapping the validation in a `try...except Exception: pass` block, which silently failed open on any unexpected error.
**Learning:** Security checks that rely on user-configurable base directories are easily bypassed if the configuration endpoint itself lacks validation. Absolute paths must be carefully sanitized to prevent assignment of system root directories. Furthermore, any security validation logic MUST "fail securely" — any unhandled exceptions during validation should result in the input being rejected, never allowed.
**Prevention:** Sanitize directory paths in configuration APIs to explicitly reject traversal characters (`..`) and system root directories (`/`, `C:\`). Always ensure validation logic uses fail-secure exception handling (`raise HTTPException(...)` instead of `pass`).
