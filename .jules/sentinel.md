...
## $(date +%Y-%m-%d) - AppleScript Command Injection Vulnerability

**Vulnerability:** Command injection when calling `osascript` in macOS because of manual string interpolation with untrusted arguments (`safe_path`).
**Learning:** Manual string replacement (e.g., escaping `\` and `"`) for AppleScript string parameters passed via the shell is fragile and prone to bypass. AppleScript itself allows execution in varied contexts, making string interpolation inherently unsafe for passing file paths.
**Prevention:** Instead of interpolating variables into the AppleScript string, pass them securely using `argv` via `subprocess.run` (e.g., `['osascript', '-e', 'on run argv', '-e', 'tell application "Finder" to move POSIX file (item 1 of argv) to trash', '-e', 'end run', path]`).
