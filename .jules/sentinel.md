...
## 2024-06-08 - Prevent Arbitrary File Execution via State Tampering in Resume Endpoint
**Vulnerability:** The `/api/resume` endpoint executed files directly via `os.startfile` or `subprocess.run` by reading a path stored in internal application settings (`last_played_file`). This path was not validated before execution, creating a risk of Remote Code Execution (RCE) if the settings configuration file was tampered with or modified maliciously.
**Learning:** Even internal state properties like "last played file" can be a vector for execution vulnerabilities if the underlying state storage is accessible or modifiable by the user (like a local JSON config file). Paths passed to OS execution commands must always be validated regardless of their origin.
**Prevention:** Always validate file paths against a strict extension allowlist (e.g. `.mkv`, `.mp4`) before executing them via OS-level commands like `os.startfile` or `subprocess.run(['open'])`, even if the path originates from an internal cache or settings store.
