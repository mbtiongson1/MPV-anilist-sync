...
## 2025-02-25 - AppleScript Command Injection via String Interpolation
**Vulnerability:** AppleScript command injection in `src/api/router_os.py` due to using Python string interpolation (`f'tell application "Finder" to move POSIX file "{safe_path}" to trash'`) alongside manual character escaping (`replace('\\', '\\\\').replace('"', '\\"')`) within an `osascript -e` call.
**Learning:** AppleScript strings have varied parsing contexts, making manual escaping inherently dangerous and prone to bypasses (e.g., via newline characters or unhandled backslash expansions).
**Prevention:** Always pass untrusted arguments to AppleScript securely via the `argv` array rather than string interpolation. Define the script to accept arguments (`on run argv`) and pass the variables as trailing parameters to the `osascript` command (e.g., `subprocess.run(['osascript', '-e', script, arg1])`).
