...
## 2026-06-29 - Path Traversal Prevention\n**Vulnerability:** Arbitrary file deletion and directory creation via unvalidated user input in `move_to_trash` and `open_folder` endpoints.\n**Learning:** API endpoints accessing the OS file system must not blindly trust file paths passed from external clients, as these paths could be maliciously crafted (e.g. `../../etc/passwd`) to modify or leak system data.\n**Prevention:** Validate paths by generating a strict allowlist of authorized root directories (e.g., download folders), converting both the allowed directories and user paths to their absolute real paths using `os.path.realpath` (to bypass symlink manipulation), and then ensuring the resolved target path sits strictly beneath the allowed root using `os.path.commonpath` before permitting the filesystem operation.
