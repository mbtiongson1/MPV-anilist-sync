...
## 2024-05-31 - OS Command Injection via Internal State (RCE)
**Vulnerability:** The `/api/resume` endpoint was passing `last_played_file` directly to OS execution commands (`os.startfile`, `subprocess.run(['open', ...])`) without validating the file type. If the `config.json` was tampered with, this could lead to arbitrary code execution (e.g. executing a `.bat` or `.sh` script).
**Learning:** Even if a file path comes from "internal state" (like a saved setting), it must be treated as untrusted input if it's passed to an OS command, because the underlying storage (like a JSON file) could be modified by a malicious actor or another process.
**Prevention:** Always validate the file extension against a strict allowlist of expected media types (e.g., `.mkv`, `.mp4`) before passing paths to OS execution commands.
