...
## 2026-06-25 - AppleScript Command Injection via String Interpolation
**Vulnerability:** AppleScript command injection in `/api/move_to_trash` because variables were passed directly into the `osascript` command string via interpolation, and escaping backslashes/quotes is insufficient.
**Learning:** Manual escaping of strings for shell/AppleScript execution is error-prone. A naive `.replace('\\', '\\\\').replace('"', '\\"')` can still be bypassed or broken by subtle parsing differences in AppleScript's interpreter.
**Prevention:** Always use safe parameterization like AppleScript's `on run argv` block, passing untrusted inputs securely via `subprocess.run`'s argument array instead of string formatting.
