...
## 2024-08-03 - Path Traversal and System Root Access via Configured Directories
**Vulnerability:** The application allowed users to configure base and download directories to sensitive system locations like `/`, `C:\`, and containing path traversal characters like `..`.
**Learning:** Security checks that rely on user-configurable base directories can be bypassed if the configuration endpoint itself lacks validation. We must validate the path not only when it's used but when it's initially saved.
**Prevention:** Always sanitize directory paths in configuration APIs to explicitly reject traversal characters (`..`) and system root directories (`/`, `C:\`, `C:/`).
