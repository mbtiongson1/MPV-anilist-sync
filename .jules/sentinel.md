...
## 2026-07-09 - Incomplete AppleScript Command Injection Escaping Fix
**Vulnerability:** A previous fix for AppleScript command injection in `/api/move_to_trash` (`src/api/router_os.py`) attempted to escape backslashes and quotes (`abs_p.replace('\\', '\\\\').replace('"', '\\"')`) but continued to use string interpolation to construct the command (`f'tell application "Finder" to move POSIX file "{safe_path}" to trash'`).
**Learning:** Relying on manual string escaping for interpolation in command execution is fragile and often misses edge cases or complex bypasses.
**Prevention:** Always use the tool's native argument passing capabilities. For `osascript`, use the `on run argv` handler to accept variables cleanly and safely without any interpolation: `['osascript', '-e', 'on run argv', '-e', 'tell application "Finder" to ... (item 1 of argv) ...', '-e', 'end run', path]`.
