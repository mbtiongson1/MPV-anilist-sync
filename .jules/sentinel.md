...
## 2026-07-12 - SSRF via Parser Differential\n**Vulnerability:** The `/api/image` proxy used `urllib.parse` to validate hostnames against an allowlist before passing the URL to `httpx` to fetch the image.\n**Learning:** `urllib.parse` and `httpx` handle malformed URLs (e.g., containing `@` or `#` in the authority) differently. An attacker could craft a URL that `urllib.parse` evaluates as an allowed domain, but `httpx` interprets as a different (malicious) domain, leading to a Server-Side Request Forgery (SSRF) bypass.\n**Prevention:** When validating URLs to prevent SSRF via parser differentials, explicitly reject ambiguous or control characters (like `@` and `#`) within the parsed `netloc` component to ensure both parsers interpret the URL identically.
