...
## 2024-07-05 - Fix SSRF via Parser Differentials\n**Vulnerability:** The `/api/image` endpoint proxy was vulnerable to SSRF bypasses via URL parser differentials (e.g. `http://127.0.0.1#@anilist.co`).\n**Learning:** Standard library URL parsers (`urllib.parse`) often disagree with HTTP clients (`httpx`, `requests`) on how to handle malformed authority sections containing `@` or `#`. Using `in` checks on the original string can accidentally block valid image paths.\n**Prevention:** When validating URLs against allowed domains, explicitly parse the URL and block ambiguous characters (`@`, `#`) specifically within the parsed `netloc` (authority) component before proceeding with the domain allowlist check.
