...
## 2026-06-10 - Path Traversal Bypass via Configuration Validation
**Vulnerability:** Path traversal protections in application endpoints (`is_safe_path`) relying on configuration states (e.g. `base_anime_folder`) could be bypassed if the configuration endpoint itself lacked validation. An attacker could configure the root directory (like `/` or `C:\`) as the base folder, bypassing directory containment checks because the containment check compares the target against the configured root.
**Learning:** Security checks that rely on user-configurable base directories are fundamentally broken if the configuration endpoint does not validate the input. Always sanitize directory paths in configuration APIs to explicitly reject traversal characters (`..`) and system root directories (`/`, `C:\`, `C:/`).
**Prevention:** Added a strict validation function (`is_invalid_config_path`) that blocks configuring any path containing `..` or representing a system root directory. This validation is applied to all directory paths within the `save_settings` configuration endpoint, effectively failing securely.
