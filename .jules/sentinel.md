...
## 2024-07-03 - AppleScript Command Injection via String Concatenation
**Vulnerability:** The `move_to_trash` endpoint constructed an `osascript` command using string interpolation to execute the Finder `move to trash` command. Even though basic escaping (backslashes and quotes) was implemented, AppleScript's dynamic execution environment could potentially be bypassed or misinterpret poorly escaped input.
**Learning:** Using string interpolation to pass user-controlled data directly into an interpreter (like bash or AppleScript) is fundamentally fragile and prone to command injection vulnerabilities. Escaping functions are notoriously difficult to get right in all edge cases.
**Prevention:** Always use parameterized execution when available. For AppleScript via `subprocess.run`, pass the variable as an external command-line argument using `osascript -e 'on run argv...'` and the target string in the command array to securely isolate code from data.
