...
## 2025-02-12 - App.jsx reduce to for-loop optimization
**Learning:** Replaced an `Array.prototype.reduce()` call with a standard `for` loop within a `useMemo` block that iterated over potentially large anime lists to compute status counts. This micro-optimization successfully eliminated O(N) callback execution overhead and inline object destructuring/manipulation on every item, resulting in faster rendering times while maintaining code readability and satisfying the 'Bolt' constraints.
**Action:** Prefer standard `for` loops over chained or recursive functional array methods like `reduce` or `forEach` when iterating over large state datasets (like anime lists) inside performance-critical `useMemo` derivations to minimize overhead.
