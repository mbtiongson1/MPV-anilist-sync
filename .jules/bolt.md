...
## 2024-05-20 - Global Signal Map Lookups
**Learning:** In Preact applications using signals, deriving state by performing O(N) array lookups (like `.find()`) inside component event handlers or render functions can become a bottleneck when the list grows. Furthermore, computing a localized lookup map inside individual components using `useMemo` causes redundant memory allocation and rebuilds if the data is needed across multiple views.
**Action:** Centralize large array lookups into a single globally `computed` Map alongside the primary Signal in the store. This prevents redundant O(N) `.find()` iterations and redundant Map allocations across individual components. When refactoring loose equality (`==`) lookups, ensure the map is dual-keyed (e.g. numeric and string) to safely preserve loose matching behavior.
