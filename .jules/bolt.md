...
## 2024-05-27 - O(N log N) Overhead in Array.sort() with Unconditional Execution
**Learning:** In `frontend/src/components/App.jsx` and `frontend/src/components/TorrentsView.jsx`, `Array.prototype.sort()` was executing expensive functions (e.g. `getDisplayTitle()`, string `.toLowerCase()`, regex-based `parseSize()`, and `Array.indexOf()`) directly inside the sort comparator. Worse, in `App.jsx`, these functions were being executed for EVERY comparison *unconditionally*, regardless of the actual active sort criteria! This caused massive, redundant O(N log N) overhead, making list sorting sluggish.
**Action:** Always pre-compute expensive operations and format strings OUTSIDE the `Array.sort()` callback in an O(N) loop, and store the results in a `Map` (often called a Schwartzian transform). Then, inside the sort comparator, use the `Map.get()` to do fast $O(1)$ lookups. Additionally, wrap calculation logic inside `switch/case` conditions so only the necessary values for the active sort criteria are actually computed.
