...
## 2026-06-25 - Avoid Micro-Optimizing Higher-Order Array Methods in UI Renders
**Learning:** Replacing standard JavaScript higher-order array methods (like `.forEach()`, `.map()`, or `.reduce()`) with traditional `for` loops is a common engine-level micro-optimization to eliminate callback overhead. However, in modern UI frameworks (like Preact/React) and modern JS engines (V8), this callback overhead is negligible compared to DOM updates and state changes. Making code more verbose strictly to avoid `.reduce()` or nested `.forEach()` violates the principle of not sacrificing code readability for micro-optimizations that have no measurable impact on end-user performance.
**Action:** Do not refactor readable `.reduce()` or `.map()` chains into verbose `for` loops unless dealing with provably massive datasets or tight render loops where a measurable bottleneck has been profiled. Focus optimization efforts on algorithmic improvements (e.g. $O(N^2)$ to $O(N)$) or preventing redundant execution (e.g. via `useMemo`), not syntax-level micro-optimizations.
