...
## 2026-07-15 - Avoid in-place O(N log N) sorting in render loops
**Learning:** In Preact components, directly sorting state arrays in the component's functional render body (e.g., `seasonOptions.sort(...)` in `NowPlaying.jsx`) causes the array to be mutated in-place and the expensive O(N log N) sort operation to run on *every* single component render.
**Action:** Always create a shallow copy (e.g. `[...array]`) and wrap array sorting in a `useMemo` hook to prevent mutating original state and to ensure the sort only re-runs when the dependency array changes.
