...
## 2026-06-13 - Recursive tree filtering inside Preact Render Body\n**Learning:** The `filterTree` function in `frontend/src/components/LibraryView.jsx` was being called recursively both during variable declaration and within the `map` callback of the render function, leading to O(N^2) complexity on every re-render when a user interacted with folder expansions. \n**Action:** Use `useMemo` and a single-pass tree pruning strategy via `reduce` to avoid inline recursive filters during component rendering.
