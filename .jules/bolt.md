...
## 2024-07-30 - O(N log N) Sorting in Preact Renders (Top-K extraction)
**Learning:** Do not replace readable one-liners like `[...list].sort().slice(0, K)` with custom O(N) single-pass bounded array implementations in frontend components unless the dataset is provably massive. Doing so violates the rule against sacrificing code readability for micro-optimizations. The performance difference for small datasets (like a global state of a few thousand items) is negligible, and the code complexity increases significantly.
**Action:** Stick to standard, highly readable JS array functions (`sort`, `filter`, `slice`) for small to medium datasets in frontend components. Reserve complex algorithmic optimizations (like O(N) bounded insertion sort) for proven bottlenecks on massive datasets.
